...
CREATE TABLE IF NOT EXISTS public.transactions (
  ...
  notes          TEXT,
  subcategory     TEXT,
  payment_method TEXT DEFAULT 'cash' CHECK (payment_method IN ('cash', 'credit_card', 'debit_card', 'bank_transfer', 'check', 'mobile_payment', 'other')),
  created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.user_settings (
  ...
  notifications JSONB NOT NULL DEFAULT '{"budget_alerts": true, "recurring_reminders": true, "goal_milestones": false}'::jsonb,
  created_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at    TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE POLICY "backup_history_select_own" ON public.backup_history FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "backup_history_insert_own" ON public.backup_history FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "backup_history_update_own" ON public.backup_history FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "backup_history_delete_own" ON public.backup_history FOR DELETE USING (auth.uid() = user_id);
...
CREATE TRIGGER on_auth_user_seed_categories AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.seed_default_categories();
